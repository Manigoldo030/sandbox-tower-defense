# scripts/managers/base_tilemap.gd
# base.tscn の TileMapLayer にアタッチする
	...
	for y in range(MAP_H):
		for x in range(MAP_W):
			...
			if x == 0 or x == MAP_W - 1 or y == 0 or y == MAP_H - 1:
				...
				set_cell(Vector2i(x, y), 0, Vector2i(0, 0))  # 草

	# 拠点の中心に土タイルでベースエリアを示す
	for y in range(12, 18):
		for x in range(16, 24):
			set_cell(Vector2i(x, y), 0, Vector2i(1, 0))  # 土

	# フィールドへの出口（上の壁中央）
	var mid: int = MAP_W / 2
	set_cell(Vector2i(mid - 1, 0), 0, Vector2i(0, 0))
	set_cell(Vector2i(mid,     0), 0, Vector2i(0, 0))
	set_cell(Vector2i(mid + 1, 0), 0, Vector2i(0, 0))
